...
public class Queen extends ChessPiece{
	...
	public boolean[][] possibleMoves() {
		...
		// Testa movimentos para cima (acima da torre)
		...
		// Testa movimentos para a esquerda
		...
		// Testa movimentos para a direita
		...
		// Testa movimentos para diagonal nordeste
				...
				// Testa movimentos para diagonal noroeste
				// Define a nova posição uma linha acima e uma coluna à esquerda
				p.setValues(position.getRow() -1, position.getColumn() - 1);
				// Loop enquanto a posição existe e não há peça nela
				while(getBoard().positionExists(p) && !getBoard().thereIsAPiece(p)){
					// Marca esta posição como movimento possível
